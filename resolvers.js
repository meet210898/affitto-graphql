...
const resolvers = {
    Query: {
        user: async () => await User.find({}).populate("stateId", "_id stateName").populate("cityId", "_id cityName"),
        ...
        company: async () => await Company.find({}).populate("typeId", "_id typeName"),
        vehicle: async () => await Vehicle.find({}).populate("typeId", "_id typeName").populate("companyId", "_id companyName"),
        booking: async () => await Booking.find({})
        .populate("userId", "_id firstName lastName")
        .populate("companyId", "_id companyName")
        .populate("vehicleId", "_id vehicleName"),
        faqCategory: async () => await FaqCategory.find({}),
        faq: async () => await Faq.find({}).populate("faqCategoryId", "_id faqCategory"),
    },
    Mutation: {
        ...
        createState: async (_, { stateName, stateImage }, { userId }) => {
            if(!userId){
        ...
        createCity: async (_, { stateId, cityName, cityImage }, { userId }) => {
            if(!userId){
                throw new Error("You must be logged in")  
            }
        ...
        createFaqCategory: async (_, { faqCategory }, { userId }) => {
            if(!userId){
                throw new Error("You must be logged in")  
            }
